...
class Node
{
    ...
    /**
     * Finding cache in tree
     * @param key - sumBytes
     * @param bytes - serialized object
     * @return - found bytes of null
     */
    public byte[] findItem(long key, byte[] bytes)
    {
        for(int j=0; j<ORDER-1; j++)
        {
            if(itemArray[j] == null)
                break;
            else if(itemArray[j].sumBytes == key)
            {
                boolean equals = true;
                try {
                    for (int i = 0; i < bytes.length; i++) {
                        if (itemArray[j].bytes[i] != bytes[i]) {
                            equals = false;
                            continue;
                        }
                    }
                ...
                return equals ? itemArray[j].bytes : null;
            }
        }
        return null;
    }
    ...
    public int insertItem(DataItem newItem)
    {
        ...
        long newKey = newItem.sumBytes; // Ключ нового элемента
        ...
        { // проверяем элементы
            ...
            { // получить ее ключ
                long itsKey = itemArray[j].sumBytes;
